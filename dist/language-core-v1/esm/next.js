...
import { A as AccountId, C as ChoiceId } from './account-c787daee.js';
import { P as Party } from './party-2b7ee6d1.js';
import { T as Token } from './token-e1a1eafb.js';
import { B as Bound } from './choice-630ae815.js';
...
const IsMerkleizedContinuation = lib.boolean;

const CaseIndex = lib.bigint;

const CanDeposit = lib.type({
    case_index: CaseIndex,
    party: Party,
    can_deposit: lib.bigint,
    of_token: Token,
...
const CanChoose = lib.type({
    case_index: CaseIndex,
    for_choice: ChoiceId,
    can_choose_between: lib.array(Bound),
    is_merkleized_continuation: IsMerkleizedContinuation,
});

const CanNotify = lib.type({
    case_index: CaseIndex,
    is_merkleized_continuation: IsMerkleizedContinuation,
});

const ApplicableInputs = lib.type({
    notify: optionFromNullable(CanNotify),
    deposits: lib.array(CanDeposit),
    choices: lib.array(CanChoose),
});

const Next = lib.type({
    can_reduce: lib.boolean,
