...
var api = require('@marlowe.io/wallet/api');
var runtimeCore = require('@marlowe.io/runtime-core');
var Tx = require('@marlowe.io/runtime-rest-client/transaction');
...
const create = (client) => (wallet) => (payload) => _function.pipe(fpTs.tryCatchDefault(() => api.getAddressesAndCollaterals(wallet)), TaskEither.chain((addressesAndCollaterals) => client.contracts.post({
    ...
    minUTxODeposit: payload.minUTxODeposit
        ...
        : 3000000,
}, addressesAndCollaterals)), TaskEither.chainW((contractTextEnvelope) => _function.pipe(fpTs.tryCatchDefault(() => wallet.signTxTheCIP30Way(contractTextEnvelope.tx.cborHex)), TaskEither.chain((hexTransactionWitnessSet) => client.contracts.contract.put(contractTextEnvelope.contractId, hexTransactionWitnessSet)), TaskEither.map(() => contractTextEnvelope.contractId))), TaskEither.chainFirstW((contractId) => fpTs.tryCatchDefault(() => wallet.waitConfirmation(_function.pipe(contractId, runtimeCore.contractIdToTxId)))));
const applyInputs = (client) => (wallet) => (contractId) => (payload) => _function.pipe(fpTs.tryCatchDefault(() => api.getAddressesAndCollaterals(wallet)), TaskEither.chain((addressesAndCollaterals) => client.contracts.contract.transactions.post(contractId, {
...
}, addressesAndCollaterals)), TaskEither.chainW((transactionTextEnvelope) => _function.pipe(fpTs.tryCatchDefault(() => wallet.signTxTheCIP30Way(transactionTextEnvelope.tx.cborHex)), TaskEither.chain((hexTransactionWitnessSet) => client.contracts.contract.transactions.transaction.put(contractId, transactionTextEnvelope.transactionId, hexTransactionWitnessSet)), TaskEither.map(() => transactionTextEnvelope.transactionId))), TaskEither.chainFirstW((transactionId) => fpTs.tryCatchDefault(() => wallet.waitConfirmation(_function.pipe(transactionId, Tx__namespace.idToTxId)))), TaskEither.map(() => contractId));
const withdraw = (client) => (wallet) => (payoutIds) => _function.pipe(fpTs.tryCatchDefault(() => api.getAddressesAndCollaterals(wallet)), TaskEither.chain((addressesAndCollaterals) => client.withdrawals.post(payoutIds, addressesAndCollaterals)), TaskEither.chainW((withdrawalTextEnvelope) => _function.pipe(fpTs.tryCatchDefault(() => wallet.signTxTheCIP30Way(withdrawalTextEnvelope.tx.cborHex)), TaskEither.chain((hexTransactionWitnessSet) => client.withdrawals.withdrawal.put(withdrawalTextEnvelope.withdrawalId, hexTransactionWitnessSet)), TaskEither.map(() => withdrawalTextEnvelope.withdrawalId))), TaskEither.chainFirstW((withdrawalId) => fpTs.tryCatchDefault(() => wallet.waitConfirmation(_function.pipe(withdrawalId, runtimeCore.withdrawalIdToTxId)))), TaskEither.map(_function.constVoid));
